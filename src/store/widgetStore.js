...
export const useWidgetStore = create((set, get) => ({
  ...
  toggleWidgetAi: (matchId) => {
    ...
    if (next) {
      const match = get().matches.find(m => m.id === matchId) || get().currentMatch;
      set({ 
        panelOpen: false, 
        widgetAiOpen: next, 
  ...
  sendWidgetAiMessage: async (text) => {
    ...
    try {
      const match = get().matches.find(m => m.id === widgetAiMatchId) || get().currentMatch;
      ...
      set({
        widgetAiMessages: [...updatedMsgs, { sender: "ai", text: response || "BEEP BOOP! CONNECTIVITY ERROR." }],
        widgetAiLoading: false
      });
    } catch (err) {
      set({
        widgetAiMessages: [...updatedMsgs, { sender: "ai", text: `ERROR: ${err.message}` }],
        widgetAiLoading: false
      });
    }
  },
  ...
  openAiCommentary: (matchId) => {
    set({ panelOpen: true, widgetAiOpen: false, expandedMatchId: matchId, activeSubTab: "ai" });
    window.electronAPI?.setPanelOpen(true, get().viewMode);
  },

  setMatches: (matches) => {
    const { customTheme } = get();
    const filtered = applyFiltersAndSorting(matches, customTheme);
  ...
  nextMatch: () => {
    ...
    let safeIndex = typeof currentMatchIndex === "number" && !isNaN(currentMatchIndex) ? currentMatchIndex : 0;
  ...
  goToMatch: (idOrIdx) => {
    ...
    if (idx < 0 || idx >= matches.length) {
      ...
      const foundIdx = currentMatch ? matches.findIndex((m) => m.id === currentMatch.id) : 0;
  ...
  setCustomTheme: (theme) => {
    ...
    const filtered = applyFiltersAndSorting(get().rawMatches, updated);
    const currentMatch = filtered[get().currentMatchIndex] || filtered[0] || null;
    set({
      customTheme: updated,
      matches: filtered,
...
function applyFiltersAndSorting(matches, theme) {
  if (!matches) return [];
  ...
  // 2. Sort by favorite teams (prioritized first, maintaining chronological sequence)
  if (theme?.favoriteTeams && theme.favoriteTeams.length > 0) {
    filtered.sort((a, b) => {
      ...
      if (!aFav && bFav) return 1;
      return new Date(a.kickoff) - new Date(b.kickoff); // preserve normal order
    });
  }
